...
describe 'a site visitor' do
  ...
  context 'who is not logged in' do
    it 'can not see anything on other users show pages' do
      ...
      expect(page).to have_content("Create an account to see your own home page")
    end
  end
  context 'who is logged in as a default user' do
    it 'can navigate with the navbar to certain user pages' do
      ...
      expect(page).to have_content("Click on the links below to see what other user's favorited.")
      ...
      expect(current_path).to eq(user_favorites_path(@user2))
    end
  end
